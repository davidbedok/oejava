...
public class SizeSearchCriteria {
	...
	public boolean isValid(Size size) {
		boolean valid = true;
		if (size != null) {
			valid = (DoubleRange.isValid(this.widthRange, size.getWidth())
					&& DoubleRange.isValid(this.heightRange, size.getHeight()) && DoubleRange
					.isValid(this.lengthRange, size.getLength()));
		}
		return valid;
	}
	...
	public String toString() {
		return "SizeSearchCriteria: widthRange: " + this.widthRange
				+ " heightRange: " + this.heightRange + " lengthRange: "
				+ this.lengthRange;
	}
