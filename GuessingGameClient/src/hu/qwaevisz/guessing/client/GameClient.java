...
public class GameClient {
	...
	public GameClient() throws UnknownHostException, IOException {
		Socket socket = new Socket(ConnectionConstants.SERVER_HOST,
				ConnectionConstants.SERVER_PORT);
	...
	private boolean processOperation(Operation operation) throws IOException {
		...
		case SERVER_WELCOME:
			...
			this.socketWrapper.send(Operation.CLIENT_INTRODUCTION,
					"Hello! I'm " + name + ".");
			break;
	...
	private boolean tip() {
		...
		if (isKill) {
			...
			this.socketWrapper.send(Operation.CLIENT_TIP, Integer.valueOf(tip)
					.toString());
		}
		return isKill;
	}
